...
class Siswa8b extends CI_Controller {

	public function __construct() 
	{
		parent::__construct();
		$this->load->model('Siswa8b_Model');
	...
	function index() 
	{
		...
		$data['judul'] = 'Daftar Siswa 8-B';
		$this->load->view('admin/templates/header', $data);
		$this->load->view('siswa8b/index', $data);
		$this->load->view('admin/templates/footer');
	}

	function load_data()
	{
		$data = $this->Siswa8b_Model->load_data();
	...
	function insert()
	{
		...
		$this->Siswa8b_Model->insert($data);
	}
	...
	function update()
	{
		...
		$this->Siswa8b_Model->update($data, $this->input->post('nisn'));
	}

	function delete()
	{
		$this->Siswa8b_Model->delete($this->input->post('nisn'));
	}

	function chart()
	{
	  $data1['judul'] = 'Daftar Guru';
	  $data['hasil']=$this->Siswa8b_Model->graph();
	  $this->load->view('admin/templates/header', $data1);
	  $this->load->view('siswa8b/chart',$data);
	  $this->load->view('admin/templates/footer', $data1);
	}

	public function export_excel(){
		$user = $this->Siswa8b_Model->load_data();
	...
	header('Content-Disposition: attachment;filename="Laporan Data Siswa 8-B.xlsx"');
	...
	function export_pdf(){
        ...
        $pdf->Cell(0,9,'Laporan Data Siswa Kelas 8-B',0,1,'C');
        $pdf->Cell(10,9,'',0,1);
