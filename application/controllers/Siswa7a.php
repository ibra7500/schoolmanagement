...
class Siswa7a extends CI_Controller {

	public function __construct() 
	{
		parent::__construct();
		$this->load->model('Siswa7a_Model');
	...
	function index() 
	{
		...
		$data['judul'] = 'Daftar Siswa 7-A';
		$this->load->view('admin/templates/header', $data);
		$this->load->view('siswa7a/index', $data);
		$this->load->view('admin/templates/footer');
	}

	function load_data()
	{
		$data = $this->Siswa7a_Model->load_data();
	...
	function insert()
	{
		...
		$this->Siswa7a_Model->insert($data);
	}
	...
	function update()
	{
		...
		$this->Siswa7a_Model->update($data, $this->input->post('nisn'));
	}

	function delete()
	{
		$this->Siswa7a_Model->delete($this->input->post('nisn'));
	}

	function chart()
	{
	  $data1['judul'] = 'Daftar Guru';
	  $data['hasil']=$this->Siswa7a_Model->graph();
	  $this->load->view('admin/templates/header', $data1);
	  $this->load->view('siswa7a/chart',$data);
	  $this->load->view('admin/templates/footer', $data1);
	}

	public function export_excel(){
		$user = $this->Siswa7a_Model->load_data();
	...
	header('Content-Disposition: attachment;filename="Laporan Data Siswa 7-A.xlsx"');
	...
	function export_pdf(){
        ...
        $pdf->Cell(0,9,'Laporan Data Siswa Kelas 7-A',0,1,'C');
        $pdf->Cell(10,9,'',0,1);
